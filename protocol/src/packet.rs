...
pub fn read_header(t: &mut TcpStream) -> std::io::Result<(u32, u8)> {
...
impl Handshake {
    pub fn new(protocol_number: u8) -> Handshake {
        Handshake { protocol_number }
    }
    pub fn read(t: &mut TcpStream) -> std::io::Result<Handshake> {
        let (_len, _id) = read_header(t)?;
        let protocol_number = read_u8(t)?;
        Ok(Handshake::new(protocol_number))
    }
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        write_vec_u8(&mut bytes, self.protocol_number);
        bytes
    }
}
