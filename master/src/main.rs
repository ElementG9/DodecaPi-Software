...
fn main() -> std::io::Result<()> {
    ...
    let _ = Handshake::new(CLIENT_PROTOCOL_VERSION, 0x01).write(&mut stream)?;
    ...
    // Write 0x05 Factor Request
    println!("Sending 0x05 Factor Request");
    let _ = FactorRequest::new(37, 0, 16).write(&mut stream)?;
    // Read 0x06 Factor Response
    let (_packet_len, packet_id) = read_packet_header(&mut stream)?;
    if packet_id == 0x07 {
        // Got a 0x07 Disconnect
        println!("Server sent a disconnect. Disconnecting...");
        disconnect(&mut stream)?;
        return Ok(());
    } else if packet_id != 0x06 {
        // If not 0x06 Factor Response
        println!("Didn't get 0x06 Factor Response when expected. Disconnecting...");
        disconnect(&mut stream)?;
        return Ok(());
    }
    let response = FactorResponse::read(&mut stream)?;
    println!("Got 0x06 Factor Response");
    println!("{:?}", response);
    // // Write 0x09 Ping
